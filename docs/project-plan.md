...
**问题**：商品列表页的分类导航、类目落地页和筛选配置，是否继续以 Magento 类目体系为前台主结构？

**现阶段判断**：不建议继续以 Magento 原始类目树直接驱动前台体验。下一阶段应明确采用“Strapi 驱动的商品发现体系”：

- Strapi 负责前台分类树、类目落地页、筛选配置、排序配置、图标/Banner/SEO 等运营与展示层能力
- Magento 继续负责商品主数据、价格、库存、可售状态和原始分类关系等交易事实
- Next.js 负责聚合 Strapi 配置与商品事实，输出统一的分类页和筛选页体验

**边界说明**：这里的“Strapi 管分类和筛选”是指 Strapi 管理前台发现体系，不表示商品完全脱离 Magento 的原始分类事实。商品侧仍应保留 Magento 原始分类关系作为同步依据和数据校验来源；前台分类是独立的展示/运营结构，而不是对 Magento 类目树的一比一复刻。

**当前建议**：阶段三先定商品发现的统一契约，而不是先绑定某个底层检索引擎。优先明确：前台分类标识、筛选参数、排序键、URL 规则、分页语义和商品卡片字段；这样后续无论底层先走 Magento/SSO 还是逐步迁移到 Meilisearch，都不需要推翻前台结构。
