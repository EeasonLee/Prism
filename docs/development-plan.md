...
- [x] 迁移 blog 相关代码到 `libs/blog/`
  ...
  - [x] Hooks → `libs/blog/hooks/`（目录已创建，待后续业务 hooks 迁移）
- [ ] 迁移 recipe 相关代码到 `libs/recipe/`（待后续任务）
- [x] 更新所有引用（blog 相关）
- [x] 验证模块边界规则（ESLint）

**验收标准：**

- ✅ 所有业务代码按域清晰分离（blog 域已完成）
- ✅ 模块边界规则通过 ESLint 检查
- ✅ 应用功能正常
- ⏳ recipe 域待后续迁移
...
- ✅ `libs/blog/` 完整结构
  - ✅ `libs/blog/api/` - API 类型和查询函数
  - ✅ `libs/blog/components/` - 业务组件
  - ✅ `libs/blog/hooks/` - Hooks 目录（待后续业务 hooks）
  - ✅ `libs/blog/src/index.ts` - 统一导出
- ⏳ `libs/recipe/` 完整结构（待后续任务）

**完成情况：**

- ✅ Blog 业务域迁移完成
  - ✅ 所有 API 类型和函数已迁移到 `libs/blog/api/`
  - ✅ 所有业务组件已迁移到 `libs/blog/components/`
  - ✅ 所有引用已更新为使用 `@prism/blog` 路径别名
  - ✅ 模块边界规则已配置并通过 ESLint 验证
  - ✅ 构建和类型检查通过
- ⏳ Recipe 业务域待迁移（当前仍在 `apps/prism/app/recipes/`）

---

#### 任务 1.6: 创建 Import 路径迁移工具（Day 5）⏸️ 延后
...
- Blog 域迁移已完成，已使用路径别名
- Recipe 域延后处理，暂时不需要批量迁移
- 剩余文件数量少（约 10 个），手动处理更高效
- 符合 YAGNI 原则，避免过度工程化

**任务清单：**

- [ ] 创建 `tools/migrate-imports.js` 脚本（延后）
- [ ] 实现相对路径到别名的转换逻辑（延后）
- [ ] 添加验证机制（转换后类型检查）（延后）
- [ ] 编写使用文档（延后）
...
- `tools/migrate-imports.js`（延后）
- `tools/README.md`（工具使用说明）（延后）

**后续计划：**

- 如果 Recipe 域迁移时文件数量较多，再考虑创建工具
- 否则在 Recipe 迁移时手动处理相对路径

---

#### 任务 1.7: 执行 Import 路径迁移（Day 6-7）⏸️ 延后
...
- Blog 域已迁移完成，已使用路径别名（`@prism/blog`、`@prism/ui`）
- Recipe 域延后处理，将在 Recipe 迁移时一并处理相对路径
- `libs/blog` 内部组件使用相对路径符合规范（同一库内允许）
- 剩余少量相对路径（约 10 个）可随 Recipe 迁移时处理

**任务清单：**

- [x] Blog 域相对路径已迁移（已完成）
- [ ] Recipe 域相对路径迁移（延后，随 Recipe 迁移时处理）
- [ ] 其他文件相对路径迁移（延后，随 Recipe 迁移时处理）

**迁移顺序：**

1. ✅ `libs/shared/`（已完成）
2. ✅ `libs/blog/`（已完成）
3. ⏸️ `libs/recipe/`（延后，随 Recipe 迁移时处理）
4. ⏸️ `apps/prism/` 中剩余文件（延后，随 Recipe 迁移时处理）

**验收标准：**

- ✅ Blog 域：所有 import 使用路径别名
- ⏸️ Recipe 域：待迁移时处理
- ✅ 类型检查通过
- ✅ 应用功能正常
...
- [x] 创建通用响应类型（`libs/shared/api/types/common.ts`）
  - [x] `ApiResponse<T>`
  - [x] `PaginatedResponse<T, M>`
  - [x] `ApiErrorResponse`
- [x] 创建错误类型体系（`libs/shared/api/types/errors.ts`）
- [x] 创建类型工具函数（类型守卫、类型提取等）
  - [x] `libs/shared/src/api/types/utils.ts` - API 类型工具
  - [x] `libs/shared/src/types/utility.ts` - 工具类型
  - [x] `libs/shared/src/types/state.ts` - 状态管理类型
  - [x] `libs/shared/src/types/guards.ts` - 类型守卫
- [ ] 更新现有 API 函数使用新类型（部分完成，待逐步迁移）
- [x] 编写类型使用文档（`docs/architecture/typescript-standards.md`）
...
- ✅ `libs/shared/api/types/common.ts` - 通用响应类型
- ✅ `libs/shared/api/types/errors.ts` - 错误类型体系
- ✅ `libs/shared/api/types/utils.ts` - API 类型工具函数
- ✅ `libs/shared/src/types/utility.ts` - 工具类型（DeepReadonly、DeepPartial 等）
- ✅ `libs/shared/src/types/state.ts` - 状态管理类型（AsyncState、PaginatedState 等）
- ✅ `libs/shared/src/types/guards.ts` - 类型守卫函数
- ✅ `docs/architecture/typescript-standards.md` - 完整的 TS 类型体系设计文档

**完成情况：**

- ✅ 建立了完整的类型体系架构，涵盖接口、业务、UI 组件、状态管理等层面
- ✅ 提供了类型工具函数和类型守卫，增强类型安全性
- ✅ 编写了详细的类型使用规范和最佳实践文档
- ⏳ 现有 API 函数逐步迁移使用新类型（不影响功能，可渐进式迁移）

---

#### 任务 2.2: 强化 ESLint 规则（Day 3）🔄 进行中

**目标：** 将关键规则升级为 error，添加自定义规则

**任务清单：**

- [x] 更新 `eslint.config.mjs`，将 `warn` 改为 `error`
  - [x] `@typescript-eslint/no-explicit-any: error`
  - [x] `@typescript-eslint/no-non-null-assertion: error`
- [x] 添加新的严格规则
  - [x] `@typescript-eslint/no-floating-promises: error`（需要类型信息，配置中）
  - [x] `@typescript-eslint/await-thenable: error`（需要类型信息，配置中）
- [ ] 创建自定义规则（或使用规则配置）
  - [ ] 禁止在组件中直接调用 API（待实现）
  - [ ] 强制使用路径别名（超过 2 层相对路径）（待实现）
- [ ] 更新所有现有代码以符合新规则（需要修复现有代码中的问题）
...
- [x] 更新 `.husky/pre-commit`
  - [x] 添加类型检查（必须通过）
  - [x] 添加 ESLint 检查（零警告）
- [x] 更新 `.github/workflows/ci.yml`
  - [x] 确保所有检查必须通过
  - [x] 添加模块边界检查
- [x] 测试 Git Hooks 是否正常工作
- [x] 测试 CI 流程
...
- [x] 创建 `docs/architecture/` 目录（已存在）
- [x] 编写 `docs/architecture/overview.md`（架构概览）
- [x] 编写 `docs/architecture/module-boundaries.md`（模块边界规则）（已存在）
- [x] 编写 `docs/architecture/import-rules.md`（导入规范）（已存在）
- [x] 编写 `docs/architecture/typescript-standards.md`（TS 规范）
- [x] 创建架构决策记录（ADR）模板
...
- [x] 创建 `docs/development/` 目录（已存在）
- [x] 编写 `docs/development/getting-started.md`（快速开始）
- [x] 编写 `docs/development/code-organization.md`（代码组织）
- [x] 编写 `docs/development/api-integration.md`（API 集成指南）
- [x] 编写 `docs/development/checklist.md`（开发检查清单）
- [ ] 创建示例代码（`docs/examples/`）（可选，待后续补充）
...
Phase 1 (基础架构)
  ...
  ├─ 1.2 路径别名体系 ──┐
  ├─ 1.3 libs 目录结构 ──┤
  ├─ 1.4 迁移共享代码 ──┼─→ 1.5 迁移业务代码（Blog 域完成）
  │                      │
  │                      └─→ 1.6 迁移工具 ⏸️ 延后
  │                      └─→ 1.7 执行迁移 ⏸️ 延后（Blog 域已完成，Recipe 域延后）
  │
  └─ Week 2 (基础规范)
      ├─ 2.1 TS 类型体系
      ├─ 2.2 ESLint 规则
