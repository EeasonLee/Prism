# 开发指南

本目录包含 Prism 项目的开发指南和最佳实践。

## 📚 文档索引

### 入门指南

- [快速开始](./getting-started.md) - 新成员快速上手指南
- [开发环境设置](../dev-env.md) - 开发环境配置说明

### 开发规范

- [代码组织](./code-organization.md) - 代码文件组织方式和规范
- [API 集成指南](./api-integration.md) - 如何集成新的 API 接口
- [CMS 页面开发指南](./cms-page-development-guide.md) - CMS 页面配置系统实施清单与落位说明
- [SEO 专题文档](./seo-guide.md) - Blog/Recipes SEO 的实现方式、待改进项与验收方法
- [组件开发规范](./component-standards.md) - 组件开发的最佳实践
- [业务逻辑组织](./business-logic.md) - 业务逻辑的提取和组织方式

### 检查清单

- [开发检查清单](./checklist.md) - 开发过程中的检查项

---

## 📝 文档维护

- 开发指南需与实际开发流程保持一致
- 发现新的最佳实践应及时更新文档
- 新成员反馈的问题应补充到相应文档

---

**最后更新：** 2024-12-19
